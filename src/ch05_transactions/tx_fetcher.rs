...
impl TxFetcher {
    pub fn get_url(testnet: bool) -> String {
        if testnet {
            format!("http://testnet.programmingbitcoin.com/")
        } else {
            format!("http://mainnet.programmingbitcoin.com/")
        }
    }

    pub async fn fetch(tx_id: &str, testnet: bool, fresh: bool, cache: &mut Option<HashMap<String, Transaction>>) -> Result<Transaction, reqwest::Error> {
